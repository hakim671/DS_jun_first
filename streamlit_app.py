...
if st.button("Прогноз"):
    ...
    if y_score >= 0.35:
        st.error(f"⚠️ **Высокая вероятность заболевания ({round(y_score, 2)*100}%)!** Обратитесь к специалисту.")
    else:
        st.success(f"✅ **Вы здоровы!** Вероятность заболевания низкая ({round(y_score, 2)*100}%).")
